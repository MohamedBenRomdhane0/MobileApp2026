...
- **2026-08-05** — **HomeScreen Summary + Activities cards (H1k).** Added a two-card side-by-side row after the LiveNowCard and before the SubscribeBanner. **SummaryCard** (left): dark navy glass card (`C.navy`, `borderRadius: 24`) with "Summary" title + circular glass arrow button, three stat rows (Subjects — teal dot + count from `materials.length`, Books — purple dot + count from `homeBooks.length`, Live — red dot + mock count 2), each row has a colored 3px underline bar. **ActivitiesCard** (right): same dark glass card with "This Month" subtitle + "Activities" title + arrow button, a 7×5 weekly heatmap grid (S M T W T F S) with dots of varying sizes (7–14px, pre-computed in `MOCK_WEEKLY_ACTIVITY` constant) — active dots use `#6366F1` indigo, inactive dots use `C.muted` at 30% opacity. Both cards use `useHomeCardEntrance(CARD_INDEX=7/8)` for staggered fade-rise and `useHomeCardPress` for spring scale feedback. New files: `src/screens/home/components/SummaryCard.tsx`, `src/screens/home/components/ActivitiesCard.tsx`. New types `SummaryCardProps`/`ActivitiesCardProps`/`SummaryStat`/`ActivityDot` in `HomeScreen.type.ts`; new `MOCK_WEEKLY_ACTIVITY` constant in `HomeScreen.constants.ts`; new `summaryRow`/`summaryCard`/`summaryStat*`/`activitiesCard`/`heatmap*` styles in `HomeScreen.styles.ts`. 7 new i18n keys added to ar/en/fr `home.ts` (parity kept at 67 lines each). Type-check 64 (baseline, 0 in touched files).
