...
## Changelog
- **2026-08-04** — **HomeHero redesigned: greeting + emoji quick actions.** The stat strip (already commented out) was replaced with a new hero layout. Top row keeps the avatar + LanguageSwitcher + bell and adds a glass circular search button (`onSearch` → Books). Below it: a daily-streak pill (yellow/orange `LinearGradient` via `HOME_TOKENS.streakGradient`, `home.streak_label` = "⭐ + DT 5"), a right-aligned greeting section (subtitle `home.welcome_back` = "👋 مرحباً بعودتك" + large bold child name — new `childName` prop replaces the old composed `greeting`), and a glassmorphism row (`BlurView`, `expo-blur` installed) of 5 circular emoji reactions (`HERO_EMOJIS` 😍😊😳🥳😲), each semi-transparent with a press-scale animation via `useHomeCardPress`/`useHomeCardEntrance`. Dead code removed: `HOME_STATS` constant, `HomeStat`/`HomeStatMeta` types, `home.stats.*` locale keys, `statStrip`/`statTile*` styles. Type-check still 77 (baseline).
