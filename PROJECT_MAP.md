...
- **2026-08-04** — **Liquid Glass Tab Bar (M05).** Replaced the pill-expanding tab bar with a premium iPhone Liquid Glass design: floating bar (90% screen width, 78px height, 40px radius, 22px bottom spacing), frosted blur background with multiple translucent layers, dynamic specular shimmer, and a morphing cyan capsule that slides smoothly between tabs. Tabs: Settings (Profil), Home (Accueil), Books (Livres), LearnCalendar (Écoles). Active tab icon scales to 1.2× with white color; inactive tabs at 60% opacity. Haptic feedback on tap (expo-haptics). Spring physics (damping 18, stiffness 180). Architecture: `LiquidGlassTabBar.tsx`, `LiquidGlassTab.tsx`, `AnimatedGlassBackground.tsx`, `ActiveCapsule.tsx`, `liquidTheme.ts`. Dead prototypes (`MainTabsAnimated.tsx`, `components/AnimatedTabBar/`) removed. Added `@styles` path alias to `tsconfig.json` + `babel.config.js`. Type-check ~65 (baseline).
