...
- **2026-08-10** — **Wallet bottom sheet redesign (M06).** Redesign the wallet bottom sheet to match the provided dark premium image design. New layout: dark navy sheet background (`#1A1A2E`) with purple glows, "Set Amount" header with chevron, large amount display (`$100` default) with +/- stepper buttons, preset recharge chips (`$50`, `$100`, `$500`, `$1000`) in purple active state, swipe-to-topup animated slider, and "Saved Cards" section with gradient cards + "View All" link. Files updated: `WalletBottomSheet.tsx` (complete rewrite with `SwipeToTopup` + `SavedCard` sub-components, removed balance display and payment form flow), `wallet.constants.ts` (new purple accent colors, `WALLET_SWIPE_GRADIENT`, `WALLET_CARD_GRADIENT`, amounts `[50,100,500,1000]`), `wallet.styles.ts` (dark premium theme, new `setAmountRow`/`amountStepper`/`chip`/`swipeTrack`/`savedCard*` styles), `RechargeAmountCard.tsx` (chips now show just the amount), `PaymentMethodCard.tsx` (purple accent instead of cyan), `i18n` keys for `set_amount`, `saved_cards`, `view_all`, `swipe_to_topup`, `decrease/increase_amount` (ar/en/fr parity). Removed: unused `balance` prop from `WalletBottomSheetProps` and `HomeScreen` call, unused `WALLET_CYAN` from styles import. Type-check 64 (baseline, 0 in touched files).
