...
## Changelog
- **2026-08-03** — **Bottom tab bar redesigned to an expanding-pill navigation animation.** `src/navigation/tabs/AnimatedTabBar/AnimatedTabBar.tsx` rewritten: the old sliding notch bubble/cradle (`NotchTabBar`, `BUBBLE_SIZE`, `NOTCH_DEPTH`, `activeX`, `centers`/`handleLayout`) is gone, replaced by `PillTabBar` + memoized `TabPill`. Inactive tabs collapse to icon-only (`TAB_COLLAPSED_W=54`); the active tab expands into a filled accent pill (`colors.primary`) that reveals its label. Geometry: `ICON_SIZE=22`, `BAR_HEIGHT=70`, `TAB_HEIGHT=46`, `BAR_PAD_H=14`, `TAB_ACTIVE_MAX_W=168` (expanded pill takes whatever the collapsed tabs leave behind), spring `{damping:18, stiffness:190, mass:0.9}`. A shared `pos` shared value (route name → visual index) springs on tab change; every `TabPill` derives its own width/opacity/scale from `t = max(0, 1-|pos-index|)` so widths always sum to the bar while the pill appears to slide. Animations: pill fade+scale (0.9→1), icon crossfade (idle 1→0.8, active 0.7→1.16→1), label fade-in (t 0.35→1) + translateX 12→0. `TabDef` gained `label` (Profil / Séances / Accueil / Livres / Écoles); unused/commented tabs retained with French labels. New `styles.bar` (absolute bottom, radius 28, hairline top border, shadow), `tabShell`, `pill`, `iconBox`/`iconLayer`, `tabLabel` (white 13px, ml 7). `goTo` via `useCallback` → `navigation.navigate`. Type-check clean (77 baseline).
