...
class ToggleLevelSectionMaterialServiceController extends Controller
{
    ...
    /**
     * Toggle a service for a specific material inside a section.
     */
    public function __invoke(Request $request, int $serviceId, int $levelSectionMaterialId): JsonResponse
    {
        $request->validate(['is_enabled' => 'required|boolean']);

        try {
            $this->activationService->toggleLevelSectionMaterialService(
                $levelSectionMaterialId,
                $serviceId,
