...
class ToggleLevelSectionServiceController extends Controller
{
    use SuccessResponse, ErrorResponse;

    public function __construct(private ServiceActivationService $activationService) {}

    public function __invoke(Request $request, int $serviceId, int $levelSectionId): JsonResponse
    {
        $request->validate(['is_enabled' => 'required|boolean']);

        try {
            $this->activationService->toggleLevelSectionService($levelSectionId, $serviceId, $request->boolean('is_enabled'));
